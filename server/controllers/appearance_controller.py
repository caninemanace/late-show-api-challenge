...
def create_appearance():
    try:
        ...
        for field in required_fields:
            if field not in data:                        # Validate required fields
                return jsonify({'error': f'{field} is required'}), 400
        
        
        rating = data['rating']
        if not isinstance(rating, int) or not (1 <= rating <= 5):    # Validate rating range
            return jsonify({'error': 'Rating must be an integer between 1 and 5'}), 400
        
        
        guest = Guest.query.get(data['guest_id'])
        if not guest:                               # Validate guest exists
            return jsonify({'error': 'Guest not found'}), 404
        
        
        episode = Episode.query.get(data['episode_id'])
        if not episode:                  # Validate episode exists
